...
namespace MungFramework.Core
{
    public static class DataBase
    {
        ...
        public static bool ExistDataBase()
        {
            ...
            string systempath = $"{DataBasePath}/system.sav";

            //检查系统文件是否存在
            if (!File.Exists(systempath))
            {
                Debug.Log("系统文件不存在" + systempath);
                return false;
            }
        ...
        /// 创建数据表（会覆盖原有数据表）
        /// </summary>
        public static bool CreateDataTable(string tableName)
        {
            Debug.Log("创建数据表" + tableName);
            //检查数据库是否存在
            if (!ExistDataBase())
            {
                Debug.Log("数据库不存在，创建数据表失败" + tableName);
                return false;
            }

            DataTable dataTable = new DataTable();
            dataTable.TableName = tableName;
            return FileSystem.WriteFile(DataBasePath, tableName, DataTableFormat, JsonUtility.ToJson(dataTable));
        }
        ...
        /// 获得数据表的值
        /// </summary>
        public static (string, bool) GetValue(string tableName, string key)
        {
            var (dataTable, success) = GetDataTable(tableName);
            if (success == false)
            {
            ...
            return dataTable.GetValue(key);
        }
        /// <summary>
        /// 设置数据表的值
        /// </summary>
        public static bool SetValue(string tableName, string key, string value)
        {
            var (dataTable, success) = GetDataTable(tableName);
            if (success == false)
            {
            ...
            dataTable.SetValue(key, value);
            return FileSystem.WriteFile(DataBasePath, tableName, DataTableFormat, JsonUtility.ToJson(dataTable));
        }
        /// <summary>
        /// 批量设置数据表的值
        /// </summary>
        public static bool SetValue(string tableName, List<KeyValuePair<string, string>> keyValues)
        {
            var (dataTable, success) = GetDataTable(tableName);
            if (success == false)
            {
            ...
            {
                dataTable.SetValue(keyValue.Key, keyValue.Value);
            }
            return FileSystem.WriteFile(DataBasePath, tableName, DataTableFormat, JsonUtility.ToJson(dataTable));
        }
        ...
        /// 移除键
        /// </summary>
        public static bool RemoveKey(string tableName, string key)
        {
            var (dataTable, success) = GetDataTable(tableName);
            if (success == false)
            {

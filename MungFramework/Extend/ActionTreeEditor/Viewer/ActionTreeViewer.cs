...
namespace MungFramework.ActionTreeEditor
{
    ...
    public class ActionTreeViewer : GraphView
    {
        ...
        public ActionTreeViewer()
        {
            ...
            var styleSheet = AssetDatabase.LoadAssetAtPath<StyleSheet>("Assets/Plugins/MungFramework/Extend/ActionTreeEditor/Viewer/ActionTreeViewer.uss");
        ...
        private GraphViewChange OnGraphViewChanged(GraphViewChange change)
        {
            if (change.elementsToRemove != null)
            {
                foreach (var element in change.elementsToRemove)
                {
                    ...
                    if (nodeView != null)
                    {
                        NodeTree.DeleteNode(nodeView._Node);
                    }
                    ...
                    if (edge != null)
                    {
                        ...
                        NodeTree.RemoveChild(startNodeView._Node, endNodeView._Node);
                    }
                }
            }
            ...
            if (change.edgesToCreate != null)
            {
                foreach (var edge in change.edgesToCreate)
                {
                    ...
                    {
                        NodeTree.SetNextChild(startNodeView._Node, endNodeView._Node);
                    }
                    else if (edge.output == startNodeView.AtTimePort)
                    {
                        NodeTree.SetAtTimeChild(startNodeView._Node, endNodeView._Node);
                    }
                }
            }
            return change;

...
namespace MungFramework.Logic.MungAnim
{
    public class MungAnimator : MungFramework.Entity.Entity
    {
        ...
        [SerializeField]
        [FoldoutGroup("Setting")]
        [ShowIf("playOnStart",true)]
        private float playOnStart_Delay;
        [SerializeField]
        [FoldoutGroup("Setting")]
        ...
        private void NextFrame()
        {
            ...
            if (nowFrameCount > nowframe.Duration)
            {
                nowFrame++;
                nowFrameCount = 0;
            }
        ...
        {
            IEnumerator playOnStartHelp()
            {
                yield return new WaitForSeconds(playOnStart_Delay);
                StartPlay(true);
            }

            if (playOnStart)
            {
                if (playOnStart_Delay < Mathf.Epsilon)
                {
                    StartPlay(true);
                }
                else
                {
                    StartCoroutine(playOnStartHelp());
                }
            }
